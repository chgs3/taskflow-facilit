...
A aplicação calcula um campo chamado `computedStatus` ao retornar uma tarefa pela API.

Isso significa basicamente que o status salvo no banco pode ser, por exemplo, `A FAZER`, mas se a tarefa tiver uma data limite anterior à data atual, a API vai retornar `computedStatus` como `ATRASADO`.

Uma tarefa marcada como `CONCLUIDO` permanece concluída, mesmo que a data limite esteja vencida.

Essa decisão evita de sobrescrever sem necessidade o status original salvo no banco e mantém a regra de negócio combinada.
